...
int tari_init(struct event_base *base, const tari_config_t *config)
{
    ...
    curl_easy_setopt(g_curl, CURLOPT_TIMEOUT_MS, g_config.timeout_ms);
    curl_easy_setopt(g_curl, CURLOPT_CONNECTTIMEOUT_MS, g_config.timeout_ms);
...
int tari_fetch_block_template(tari_template_callback_t callback, void *data)
{
     ...
     *   PowAlgo algo = 1;  // Monero = 0
     *   uint64 max_weight = 2;
     * }
     * Encoded: field 1 (varint) = 0, field 2 = 0 (default)
     */
    ...
    curl_easy_setopt(g_curl, CURLOPT_POSTFIELDSIZE, frame_len);
...
{
    /*
     * Parse protobuf NewBlockTemplateResponse
     * This is a simplified parser - production code should use proper protobuf lib
     *
     * Message fields:
     *   NewBlockTemplate new_block_template = 1;
     *   uint64 initial_sync_achieved = 2;
     *   MinerData miner_data = 3;
     *
     * NewBlockTemplate contains:
    ...
    /* Calculate merge mining hash from header */
    if (tmpl->header && tmpl->header_size > 0) {
        /* Use Keccak-256 hash of header */
                       ...
                       unsigned char **coinbase, size_t *coinbase_len)
{
    ...
    /* Field 1: reward */
    payload[plen++] = 0x08;  /* field 1, varint */
    plen += write_varint_local(payload + plen, reward);
    
    /* Field 2: fee */
    payload[plen++] = 0x10;  /* field 2, varint */
    plen += write_varint_local(payload + plen, fees);
    
    /* Field 3: height */
    payload[plen++] = 0x18;  /* field 3, varint */
    ...
    /* Extract coinbase from response - skip gRPC header */
                       ...
                       tari_submit_callback_t callback, void *data)
{
    ...
    /*
     * Build SubmitBlockRequest with MoneroPowData
     * This requires assembling the full block with PoW data
     */
    ...
    /* Field 1: Block (embedded message containing header + body) */
    /* Simplified: just concatenate header and body */
    payload[plen++] = 0x0A;  /* field 1, length-delimited */
    ...
    /* Field 2: pow_data */
    payload[plen++] = 0x12;  /* field 2, length-delimited */
...
bool tari_validate_address(const char *address)
{
    ...
    /* Tari addresses are typically 64+ characters (hex encoded public key) */
